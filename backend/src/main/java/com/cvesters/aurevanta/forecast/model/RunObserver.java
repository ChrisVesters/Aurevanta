 ...
 *
 * <p>
 * <strong>This is how a forecast can be watched without being changed.</strong> M6 needs
 * a duration per item per run to work out what the plan's spread is made of, and storing
 * those is five million numbers per forecast — so the engine hands them over as they go
 * past instead, and whoever is listening reduces them to running totals. That is the
 * whole reason a stored run can be explained years later from nothing but its seed.
 *
 * <p>
 * <strong>Called after everything that draws, and it draws nothing itself.</strong> M3b's
 * sharpest rule is that a parameter set to none must consume no randomness, because a
 * draw that changed no number would still advance the generator and silently unreplay
 * every forecast stored before it. An observer is not random and cannot break that — but
 * it is held to the same discipline for the same reason, and {@code EngineTests} asserts
 * that attaching one changes no number at all. {@link Engine#VERSION} does not move for
 * this.
 *
 * <p>
