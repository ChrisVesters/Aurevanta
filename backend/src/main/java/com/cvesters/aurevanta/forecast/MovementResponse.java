 ...
 * opposite of what the contribution ranking does and is not a contradiction. The
 * contribution ranking refuses when the *model* cannot reproduce a run — there is nothing
 * to compare with. This reports when the *question* changed, which is exactly the thing
 * worth reporting: refusing a pair because somebody adjusted the capacity would leave
 * them staring at two dates with no account of either. An engine version difference is
 * still the first kind and still refuses.
 *
 ...
 * @param simulations how many times the plan was run to answer this, published for the
 * inverse query's reason: a number that costs seven simulations should say so rather than
 * surprise somebody.
 */
