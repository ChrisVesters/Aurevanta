# The simulation engine — the design record

> **Scope.** **The simulation engine, first half** — distribution
...
> carried to the common-cause work: the shared team factor and scope uncertainty. Also excluded: calendar
> dates (the calendar), variance contribution (the contribution ranking), inverse queries (the inverse query), named resources (the resource model).
>
> **How to read this.** Decisions first, because this work is almost entirely decisions —
...
>
> **This is the product.** Every work before it existed to give this one something to
> chew on, and every work after it is a lens over its output. It is also the first
> work whose failure mode is not a bug: a wrong model produces a plausible number, and a
> plausible number is worse than an error, because nobody goes looking for it.
>
> **The simulation engine's band is knowingly too tight, and decision 12 is about saying so.** Independence is a
> lie — `../roadmap.md` measured a shared team factor moving the true P90 by 13 days on ten tasks
> — and the simulation engine does not model it. Shipping a band this product knows to be narrow, without
...
| # | Question | Decision |
|---|---|---|
| 1 | One work or two | **Two. This plan is the simulation engine**; the team factor and scope uncertainty are the common-cause model, with a plan of their own. |
...
### Decision 1 — the simulation engine and the common-cause model, and where the line falls

`../roadmap.md` says the simulation engine is oversized and should be split before work starts, and names the cut:
the simulation engine is fitting, sampling and the graph scheduler with a fixed global capacity, which is enough
to produce a real forecast; the common-cause model is the team factor and scope uncertainty. **This plan takes
...
argued that the common-cause model should be planned later, against an engine that exists — which sounds prudent
and is backwards: the roadmap's worry was that keeping them together "risks the correlation
modelling being rushed to get the work finished", and a plan written in advance is the
defence against that rather than a cause of it. The common-cause model also carries the one genuinely open
modelling question in this work — where new work attaches in a graph — and a question left
open until the week it blocks somebody is a question answered under time pressure.
`common-cause-and-scope-growth.md` settles it now.
...
is **what can be verified against a whole-plan closed form**. Everything in the simulation engine has one — a sum
...
**The common-cause model is not left without oracles, though, and saying it was would be too strong.** It has
three narrower ones — exactness for a single item under a factor, the roadmap's own published
209.4 → 222.2, and, best of all, byte-identical equivalence with this work once its two
parameters are zero. `common-cause-and-scope-growth.md` decision 8 carries them. The honest statement of the line is
therefore: **The simulation engine is verified whole and the common-cause model is verified in pieces**, so the simulation engine goes first because it
is what the pieces get bolted to and what the equivalence test measures against.

**What the simulation engine therefore is not:** a band anybody should commit to. See decision 12.
...
which is exactly what `../product-concept.md` asks for and what elicitation will want to act on.
...
**Two edge cases the schema permits and this must survive.** `P10 == P90` is accepted by the plan schema
...
The plan schema stores one current estimate per estimator per item and made a point of it: "two people who
disagree about a task are telling the simulation engine something, and it is the simulation engine that decides what to do about it."
...
are the common-cause model's subject; this is recorded there rather than solved here.

### Decision 4 — An unestimated item stays in the graph, weighing nothing

The plan schema decided that a forecast covers what is estimated and reports its coverage. **What nobody
...
invents data and needs a per-run record of what was invented, which is the plan schema's decision 5 and
...
maximum number of items that may be in flight at once. This is chosen handles's argument about the
...
Named resources, allocations and part-time availability are the resource model. One integer is the crude
version of all of it, and it is crude *visibly*, which is the property the calendar's working-day
...
says this plainly for the resource model and it is already true here: two defensible rules produce two
...
which the reporting work's sliding-date detector depends on.
...
keeps a forecast inside a request, which the plan schema's 500-item ceiling was fixed in order to allow.
...
- **Synchronous**, and the ceiling that makes it safe is arithmetic rather than hope: 500 items
  ...
  answer only if that is not enough, and it is not this work's problem to pre-solve.
...
The plan schema's decision 8 handed this work one obligation in as many words: **persist every forecast
...
- **Its resolved inputs, as a snapshot.** The live rows will not do. Items get reworded, edges
  ...
  something that no longer exists within a week. The reporting work's movement decomposition ("out 8 days: +5
...
That is worth more than it first appears: it means **anything the simulation engine did not store can be
recomputed later without being wrong**. The contribution ranking needs per-item durations correlated against project
outcome across runs; it does not need the simulation engine to have hoarded ten thousand vectors, because it can
...
headline percentiles get real columns beside it, because the reporting work's sliding-date detector compares
...
**A partially estimated plan is not refused**, at any coverage above zero — the plan schema's decision 5
...
This work ships a band that is too tight, and it knows by how much: `../roadmap.md` measured
...
The first two are properties of the engine version and are always present in the simulation engine; **The common-cause model
removes them by building what they name**, which is the cleanest possible definition of when
the common-cause model is done. The other two are properties of the plan being forecast.
...
- **A comprehensively outrun estimate returns nothing, and this needed deciding.** Nineteen to
  ...
  The remedy is a revision, which the plan schema makes a new row. It is reachable through the API, so it is
...
- **A topological pass runs once, before any sampling, and refuses a cyclic graph.** the plan schema
...
- **The priority key needs a transitive closure, and that ties it to the 500-item ceiling.**
  ...
  back. Worth revisiting at the resource model, where the priority rule stops being a tie-breaker and starts
  being the model.
...
- **One expectation in the tests was wrong before the code was**, which is worth recording
  because it is the failure mode this whole design is about: ten items at capacity four was
...
- Reported: P10, P50, P80, P90, P95, and the mean. **P80 because the calendar's confidence control is
  50/80/95** and a percentile that is not stored is a re-run to answer.
- A fixed-bucket histogram, so the reporting work can draw a curve without replaying.
...
- **`Forecast` carries a standard deviation, which the bullets do not list.** Without it the
  ...
  *and the variance*, and there was nothing to assert the variance against. The contribution ranking will want it
  too.
...
- **Step 5 inherits a question this step surfaced.** The engine refuses an edge naming an item
  it was not given, which is right; but the plan schema lets an arrow point at *archived* work, and a
  forecast only loads what is live. So step 5 has to decide what an edge into archived work
  means, and say so rather than dropping it silently — which is the mistake the plan schema's own step 4
  had to correct.
- **The one gap the coverage gate found was real.** No test drove the engine with work already
  ...
  exercised the wire between them. Decision 5 is one of this work's headline decisions, so
...
- `ForecastService.run(caller, tenant, projectId, capacity, sampleCount)`:
  - `requireMember` first, as everywhere — **any member may forecast** (the plan schema decision 6).
...
- `POST /api/projects/{projectId}/forecasts` `{capacity, sampleCount?}` → `201`.
  ...
  Addressed the way the plan schema addresses items: created and listed within a plan, read on its own.
...
**Done when** a forecast is a record rather than an event, and the reporting work's history has started
accumulating on its first day.

### As built — where it differs from the above

- **The archived-edge question step 4 handed over is answered, and it needed a limitation code
  of its own.** the plan schema lets an arrow point at work that has since been put away; a forecast only
  ...
  one thing not on the table**: that is exactly the mistake the plan schema's progress endpoint shipped and
...
- **The snapshot stores the ranges people typed, not the fitted parameters.** The bullets say
  ...
  a version bump may redefine, and make the reporting work's diff read as "somebody revised their estimate"
...
- **A latent bug in the whole test suite surfaced, and it is worth recording because it was
  ...
  tenant-owned points at a *person* — which stopped being true in the plan schema, when
...
- The P50 consistency signal surfaced per item where it is worth surfacing: a stated middle far
  from the implied one is the first thing elicitation will build on, and it costs nothing here beyond
  showing it.
- History: the runs already made, newest first, so a second forecast can be compared with the
  first by eye. That is not the reporting work's sliding-date detector; it is the list that proves the data for
  it is being kept.
- **No calendar dates anywhere.** Hours, everywhere, until the calendar — because a date needs a
...
- **The per-item P50 consistency signal is the one bullet not built**, and the plan was wrong
  ...
  *which* estimate is elicitation feedback, it belongs with elicitation, and the clean way in is for
...
- **There are five limitation codes on screen, not the four the bullets name**, because step 5
  ...
  rather than dropping it: the server is what versions ahead here, the common-cause model adds one, and silently
...
- `../roadmap.md`: mark the simulation engine done, split the the simulation engine section into the simulation engine and the common-cause model, and retire the "the simulation engine is
  oversized" thin spot by having actually split it. Record what the common-cause model inherits, including the two
  limitation codes it exists to delete.
- `../roadmap.md`: the calendar inherits the working-day assumption and the requirement that it be visible;
  the contribution ranking inherits that it may replay a run rather than store vectors.
...
- `common-cause-and-scope-growth.md` already exists and needs no close-out from here. What it does need from this
  step is a note wherever the simulation engine departed from its own bullets, because **The common-cause model's first test replays
  an the simulation engine run and asserts byte-identical output** — so anything this work did differently
...
- **`common-cause-and-scope-growth.md` got more than the note this step asked for.** The bullet above says to record
  where the simulation engine departed from its own bullets; what is actually useful to the person starting the common-cause model
...
- **The two limitation codes became the common-cause model's definition of done**, in `../roadmap.md` and in
  `common-cause-and-scope-growth.md` both. "The model is better" has no finish line; "`no_team_factor` and
...
- `p10_hours`, `p50_hours`, `p80_hours`, `p90_hours`, `p95_hours`, `mean_hours` — `numeric(14, 2)`,
  real columns because the reporting work queries them across runs.
...
**What this work must not absorb.** The team factor (the common-cause model) will look like four lines inside
the sampling loop, because it is four lines inside the sampling loop — and adding it here would
cost the whole-plan closed form that proves everything under it, and would cost the common-cause model the
equivalence test it uses to prove itself against this work. Both halves of that are why
`common-cause-and-scope-growth.md` is a separate build rather than a separate document. Calendar dates (the calendar) will look like a small conversion,
and will import a working-day assumption that this work has deliberately kept out. Both are
