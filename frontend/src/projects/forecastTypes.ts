/**
 * Everything the forecast endpoints answer, as the browser reads them.
 *
 * **Its own file because it is its own subject.** These are the shapes of six features that
 * all read one stored run — the band and its dates, what widened it, what cutting something
 * would buy, what another unit would buy, why the date moved, and what the plan's own
 * delivery history says instead. They share no type with the plan beside them: a work item's
 * identifier is the only thing that crosses, and it crosses as a string. Held together with
 * the plan's own types they were two thirds of one file that nothing read two thirds of.
 */
/**
